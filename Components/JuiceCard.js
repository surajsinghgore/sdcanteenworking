...
export default function JuiceCard(item) {
...
localStorage.setItem("itemOrder",itm._id)
  ...
  <>
   ...
   <div className={Style.JuiceImg}>
  <Link href={`/${item.item.JuiceName}`}><a> <Image src={item.item.Image} alt={item.item.ImageName} layout="fill" priority="true"/></a></Link>
   </div>
   <div className={Style.deatils}>
   <Link href={`/${item.item.JuiceName}`}><a><h1>{item.item.JuiceName}</h1></a></Link>
