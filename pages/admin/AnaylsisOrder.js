  ...
  useEffect(() => {
    ...
    if (allData.allItemsTop != undefined) {
      ...
      for (let i = 0; i < allData.allItemsTop.length; i++) {
        ...
        datasSet.push({
          label: Name,
          data: allData.allItemsTop[i].Rate,
          backgroundColor: "#1ba5e0",
        });
      }
      ...
      <div className={StyleFood.rightSideBar}>
        ...
        <div className={anaylsis.payment}>
          ...
          <div className={anaylsis.filters}>
            {/* main search */}
            <li>
              <select value={year} onChange={(e) => year1Change(e)}>
            ...
            {/* second filter */}
            <li>
              <select value={year2} onChange={(e) => year2Change(e)}>
            ...
            {/* third filter*/}
            <li>
              <select value={year3} onChange={(e) => year3Change(e)}>
                    ...
                    {allData.totalOrderSum != undefined ? (
                      <>
                        {allData.totalOrderSum.map((item, index) => {
                          return (
                            <li key={index}>
                              <div className={anaylsis.title}>
                                Total In {item.year} -{" "}
                              </div>
                              <div className={anaylsis.datas}>
                                {item.total ? (
                                  <> {item.total}</>
                                ) : item.total1 ? (
                                  <> {item.total1}</>
...
:""}
            ...
            <div className={anaylsis.chartSigle}>
              ...
              <div className={anaylsis.chartARea1}>
                {itemRate.length != 0 ? (
                  <MultiBarChart1 datas={itemRate} />
                ) : (
                  ""
                )}
              </div>
              <div className={anaylsis.allDatas}></div>
            </div>
          </div>
        </div>
      </div>{" "}
