    ...
    <>
      ...
      <div className={style.AllOrder}>
        ...
        <div className={style.topSection}>
          ...
          <h2>
            Total Order Placed: <span>{totalOrderLen}</span>
          </h2>
        </div>
        {/* filter section */}
        <div className={style.filter}>
          <li className={all ? style.active : ""} onClick={allData}>
            {" "}
            {all ? <hr /> : ""}All Orders
          </li>
          <li className={com ? style.active : ""} onClick={comData}>
            {com ? <hr /> : ""}Complete Order
          </li>
          <li className={rej ? style.active : ""} onClick={rejData}>
            {rej ? <hr /> : ""}Reject Order
          </li>
          <li className={pen ? style.active : ""} onClick={pendData}>
            {pen ? <hr /> : ""}Pending Order
          </li>
        </div>
        ...
        <div className={style.data}>
          ...
          {data != undefined ? (
            <>
              {data.map((item) => {
                return (
                  <div className={style.allItems} key={item._id}>
                    <div className={style.headingData}>
                      ...
                      {item.OrderStatus.toLowerCase() == "pending" ? (
                        ...
                        <li className={style.status}>
                          <div className={style.reject}>Reject</div>
                        </li>
                      ) : (
                        <li className={style.status}>
                          <div className={style.complete}>Complete</div>
                        </li>
                      )}
                    </div>
                      ...
                      {item.ItemsOrder.length != 0 ? (
                        <>
                          {item.ItemsOrder.map((sub) => {
                            return (
                              <div className={style.itemsMenu} key={sub._id}>
                                <li>{sub.ItemName}</li>
                                <li>{sub.Qty}</li>
                                <li>{sub.Category}</li>
                                <li>{sub.Amount}</li>
